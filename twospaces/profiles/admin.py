...
class CurrentSpeakerFilter (admin.SimpleListFilter):
  ...
  def queryset (self, request, queryset):
    if self.value() == '1':
      return queryset.filter(session__status='accepted', session__conference__slug=settings.DEFAULT_CONF).exclude(session__stype='lightning')
      
class User2Admin (UserAdmin):
  list_display = ('username', 'email', 'first_name', 'last_name', 'phone', 'current_speaker', 'is_staff', 'is_superuser')
  ...
  fieldsets = (
    ...
    ('Personal info', {'fields': ('first_name', 'last_name', 'email', 'verified_email', 'phone', 
      'website', 'avatar', 'biography')}),
  ...
  def current_speaker (self, obj):
    if obj.session_set.filter(status='accepted', conference__slug=settings.DEFAULT_CONF).exclude(stype='lightning').count() > 0:
...
class SMSForm (forms.ModelForm):
  frm = forms.CharField(label='From', max_length=50, initial=settings.TWILIO_NUMBERS[0])
  message = forms.CharField(max_length=140, widget=forms.TextInput(attrs={'style': 'width: 300px;'}))
...
class SMSAdmin (admin.ModelAdmin):
  ...
  def reply (self, obj):
    if obj.frm not in settings.TWILIO_NUMBERS:
      return '<a href="/admin/profiles/sms/add/?reply_to={}&to={}">Reply &raquo;</a>'.format(obj.id, obj.frm)
