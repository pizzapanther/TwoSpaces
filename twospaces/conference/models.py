...
class Conference (models.Model):
  ...
  cfp_open = models.DateTimeField("Call for Proposals Open", blank=True, null=True)
  cfp_closed = models.DateTimeField("Call for Proposals Closed", blank=True, null=True)
...
class Session (models.Model):
  ...
  def send_submitted (self, request):
    subject = "Talk Submission - {}".format(self.user.__str__())
  ...
  def admin_link (self):
    return '{}://{}/admin/conference/session/{}/'.format(SITE_PROTOCOL, SITE_DOMAIN, self.id)
    
  def user_link (self):
    return '{}://{}/admin/profiles/user/{}/'.format(SITE_PROTOCOL, SITE_DOMAIN, self.user.id)
...
class Invoice (models.Model):
  ...
  def Send (self):
    ...
    return '<a href="./{}/send/" onclick="return confirm(\'Are you sure you want to send this invoice?\')">Send Invoice</a>'.format(self.id)
  ...
  def send (self, request):
    ...
    send_mail(self.subject, message, settings.DEFAULT_FROM_EMAIL, [self.to], fail_silently=False)
