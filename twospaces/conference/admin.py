...
class CSVAdminMixin (object):
  def changelist_view (self, request, extra_context=None):
    ...
    if csv == '1':
      ...
      cl = CSVChangeList(request, self.model, list_display,
      ...
      generator = CSVFileGenerator(queryset=cl.get_queryset(request), tags=self.csv_rows)
...
class SessionAdmin (CSVAdminMixin, admin.ModelAdmin):
  ...
  list_display = ('name', 'conference', 'user', 'full_name','email', 'stype', 'level', 'start', 'duration', 'room', 'all_rooms', 'video', 'status')
  csv_rows = ('id', 'name', 'conference', 'user', 'full_name', 'email', 'stype', 'level', 'start', 'duration', 'status', 'admin_link', 'user_link')
