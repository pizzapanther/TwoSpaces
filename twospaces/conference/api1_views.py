...
def proposed_talks(request):
  ...
  for t in (
      ('talk-short', 'Short Talks'), ('talk-long', 'Long Talks'),
      ('tutorial', 'Tutorials'), ('lightning', 'Lightning Talks'),):
    talks.append(
        [t[1], Session.objects.filter(stype=t[0],
                                      conference__slug=conf).exclude(
                                          status='declined').order_by('name')])

  for i in range(0, len(talks)):
    talks[i][1] = ProposedReadSizzler(talks[i][1],
                                      many=True,
                                      exclude=['description']).data
...
def edit_talk(request, tid=None):
  ...
  if request.user.phone and request.user.biography:
    ...
    return Response(
        {'message': 'Fill out your speaker profile before submitting.'},
        status=400)
  ...
  if request.JSON():
    ...
    if sizzle.is_valid():
      obj = sizzle.save(conference=conf, user=request.user,)
      obj.set_duration()
      obj.save()
      if tid is None:
...
def my_talks(request):
  conf = request.GET.get('conf', '')
  queryset = Session.objects.filter(conference__slug=conf,
                                    user=request.user).order_by('name')
  talks = ProposedReadSizzler(queryset,
                              many=True,
                              exclude=['description', 'user']).data
  return Response(talks, status=200)
...
def schedule(request):
  ...
  while d <= conf.end:
    queryset = Session.objects.filter(
        ...
        start__day=d.day,
        start__year=d.year,).exclude(
            stype='lightning').order_by('start',
                                        'room__sorder').select_related('room')
